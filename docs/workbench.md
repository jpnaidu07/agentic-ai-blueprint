...
ranks only supplied/installed local application candidates,
...
The separate **Local model lifecycle** panel inside each solution covers local
deployment, held-out evaluations, CPU LoRA fine-tuning, checkpoint serving, approval
and application configuration. See [the walkthrough](local-model-lifecycle.md).
